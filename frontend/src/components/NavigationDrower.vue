<template>
    ...
    <v-navigation-drawer app v-model="drawer" :rail="rail" permanent @click="handleDrawerClick">
        ...
        <v-list density="compact" nav>
            ...
            <v-list-item
                ...
                <template v-slot:append>
                    ...
                    <v-icon style="font-size: 24px;" @click.stop="toggleRail">{{ rail ? 'mdi-chevron-right' :
            ...
            <v-list-item v-for="item in dashboardItems" :key="item.value" :prepend-icon="item.icon" :title="item.title"
                :value="item.value" :active="selectedItem === item.value"
        ...
        <template v-slot:append>
            ...
            <v-list-item v-for="item in appendItems" :key="item.value" :prepend-icon="item.icon" :title="item.title"
                :value="item.value" :active="selectedItem === item.value"
                :class="{ 'custom-list-item': true, 'selected-item': selectedItem === item.value }"
                @click="handleItemClick(item)"></v-list-item>
